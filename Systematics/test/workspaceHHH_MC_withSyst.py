...
customize.options.register('doGenJets',
                           True,
                           VarParsing.VarParsing.multiplicity.singleton,
                           VarParsing.VarParsing.varType.bool,
                           'doGenJets'
...
if customize.tthTagsOnly:
    ...
    print "Removing the non required tags"
    process.flashggTagSequence.remove(process.flashggVBFTag)
    process.flashggTagSequence.remove(process.flashggVHMetTag)
    process.flashggTagSequence.remove(process.flashggWHLeptonicTag)
    process.flashggTagSequence.remove(process.flashggZHLeptonicTag)
...
if is_signal or isForcedSignal:
...
print 'here we print the tag sequence before syst adition'
print process.flashggTagSequence
...
for tag in tagList: 
  ...
  for systlabel in systlabels:
      ...
      is_signal = reduce(lambda y,z: y or z, map(lambda x: customize.processId.count(x), signal_processes)) or isForcedSignal
